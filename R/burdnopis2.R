...
FX_tick_agg <- furrr::future_map(.x = FX_tick, .f = FXTickAggregateAndFillNA,
                                 delta = lubridate::dminutes(10), .progress = T)
future::plan(future::sequential)

SDP_full <- new("ShapeDescriptorParams", 
                Type = "compound",
                Descriptors = c("RawSubsequence", "PAADescriptor", "derivativeDescriptor",
                                "slopeDescriptor"),
...
test_res <- RknnShapeDTWParallel(refSeries = FX_tick_agg$`USDCHF-2020-02`,
                                 testSeries = FX_tick_agg[-7], 
                                 refSeriesStart = 2100, shapeDTWParams = SDP_full, includeRefSeries = F,
...
test_res_multiple_10_min <- RunMultipleShapeDTWkNN(refSeries = FX_tick_agg$`USDCHF-2020-02`,
                                            testSeries = FX_tick_agg[-7], 
                                            indicesVector = seq(from = 1, by = 15, length.out = 100), 
                                            shapeDTWParams = SDP_full, 
                                            includeRefSeries = F, 
                                            targetDistance = "r", subsequenceWidth = 15, forecastHorizon = 50,
                                            refSeriesLength = 100, sd_border = 1.5)


plot(test_res_multiple_10_min$refSeriesReturn,
     test_res_multiple_10_min$testSeriesReturn)


test_res_multiple
cor(test_res_multiple$refSeriesReturn, test_res_multiple$testSeriesReturn)
sum(test_res_multiple$kNNSuccess)
...
test_res_multiple_2 <- test_res_multiple %>%
  mutate(refClassWithoutFlat = ifelse(refSeriesReturn < 0, "Fall", "Growth"),
         testClassWithoutFlat = ifelse(testSeriesReturn < 0, "Fall", "Growth")) %>%
  filter(testSeriesReturn > 5e-04)
